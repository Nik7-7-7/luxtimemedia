...
class Envato_marketplaces {
   ...
   */
   public function display_thumbs($user_name, $marketplace_name, $limit = null)
   {
      ...
      echo esc_ulp_content("<ul class='envato-marketplace-thumbs'> \n");
      foreach($results as $item) : ?>
      <?php if ( is_null($item) ){
         break;
      } ?>
      <li>
          <a href="<?php echo esc_url($item->url) . "?ref=" . esc_attr( $user_name ); ?>" title="<?php echo esc_attr($item->item);?>">
   ...
   */
   public function search($search_expression, $marketplace_name = '', $type = '', $limit = null)
   {
      if ( empty($search_expression) ){
        ...
        $search_expression = preg_replace('/\s/', '|', $search_expression);
      }
