...
if (class_exists('ListCourseReviews')){
	 return;
}
class ListCourseReviews extends Indeed_List_Table{
	/**
	 * @var string
	 */
	protected $post_type = 'ulp_course_review';
	/**
	 * @var string
	 */
	protected $label = 'Course Review';
	/**
	 * @var string
	 */
	protected $label_plural = 'Course Reviews';
	/**
	 * @param none
	 * @return string
	 */
	...
	public function get_columns(){
	  	$columns = array(
		    ...
		    'title' =>  esc_html__('Review Title', 'ulp'),
				'target_course' =>  esc_html__('Target course', 'ulp'),
				'rating' =>  esc_html__('Rating', 'ulp'),
		    'author' =>  esc_html__('Student', 'ulp'),
	  	...
	  	switch ($column_name){
	    	case 'title':
				...
				case 'post_status':
					...
					break;
				case 'target_course':
					$course = get_post_meta($post['ID'], '_ulp_course_id', true);
					if ($course){
					...
					break;
				case 'rating':
					$rating = get_post_meta($post['ID'], '_ulp_rating', TRUE);
					echo esc_ulp_content('<div>' . ulp_generate_stars($rating) . '</div>');
					break;
	  	}
	  ...
	  return $sortable_columns;
		...
		function column_title($item) {
		  	...
		  	$actions = array(
		            ...
		            'delete'    => "<span class='js-ulp-do-delete-post ulp-delete-link' data-id='{$item->ID}'>".  esc_html__("Delete", 'ulp') . "</span>",//"<a href='" . admin_url('admin.php?page=ultimate_learning_pro&tab=ulp_course_review&action=trash&id=' . $item->ID . '&_wpnonce=' . $wpnonce) . "'>" .  esc_html__("Delete", 'ulp') . "</a>",
				...
				if (strcmp($item->post_status, 'pending')==0){
						$actions ['post_status_action'] = '<span onclick="ulpChangePostStatus('.$item->ID.', \'publish\', course_review_url);" class="ulp-pointer ulp-like-link-span ulp-change-status-link">' .  esc_html__('Publish', 'ulp') . '</span>';
				} else {
						$actions ['post_status_action'] = '<span onclick="ulpChangePostStatus('.$item->ID.', \'pending\', course_review_url);" class="ulp-pointer ulp-like-link-span ulp-change-status-link">' .  esc_html__('Pending', 'ulp') . '</span>';
				}
		...
		protected function table_data($per_page=5, $page_number=1){
				...
				if (isset($_GET['list_by_course_id'])){
						$q = "SELECT a.ID,a.post_author,a.post_date,a.post_date_gmt,a.post_content,a.post_title,a.post_excerpt,a.post_status,a.comment_status,a.ping_status,a.post_password,a.post_name,a.to_ping,a.pinged,a.post_modified,a.post_modified_gmt,a.post_content_filtered,a.post_parent,a.guid,a.menu_order,a.post_type,a.post_mime_type,a.comment_count
										...
										AND
										a.post_status NOT IN ('trash')
										AND
										b.meta_key='_ulp_course_id'
										AND
										b.meta_value={$_GET['list_by_course_id']}
						";
