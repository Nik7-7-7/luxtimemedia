/*
* Ultimate Learning Pro - Image Croppic
*/
"use strict";
var UlpAvatarCroppic = {
    triggerId                   : '',
    saveImageTarget             : '',
    cropImageTarget             : '',
    hiddenInputSelector         : '',
    ...
    handleRemove: function(obj, isReset){
        jQuery( obj.imageSelectorWrapper ).html( '' );
		    jQuery( obj.imageSelectorWrapper ).append( '<div class="ulp-no-avatar ulp-member-photo"></div>' );
		    jQuery( obj.removeImageSelector ).css('visibility', 'hidden');
        jQuery(obj.hiddenInputSelector).val( '' );
        if (isReset)
		        jQuery( '#' + obj.triggerId ).append( '<div id="ulp-avatar-button" class="ulp-upload-avatar">'+obj.buttonLabel+'</div>' );
