...
function masterstudy_get_current_url( $params_to_remove = array() ) {
	...
	$new_uri   = $parsed_url['path'];

	if ( ! empty( $new_query ) ) {
		$new_uri .= '?' . $new_query;
	}

	return home_url( $new_uri );
}

function masterstudy_lms_count_assignments_by_status( $user_assignments, $key, $status ): int {
	return count(
		array_filter(
			$user_assignments,
			function( $assignment ) use ( $key, $status ) {
				if ( 'pending' === $status ) {
					return empty( $assignment['meta_status'] ) && in_array( $assignment['post_status'], array( 'pending', 'publish' ), true );
				}

				return ( $assignment[ $key ] ?? '' ) === $status;
			}
		)
	);
}
...
function masterstudy_lms_is_user_course_owner( $course_id, $user_id ): bool {
	$course_author = get_post_field( 'post_author', $course_id );

	return (int) $course_author === (int) $user_id;
}

// Check if user is student of the instructor
function masterstudy_lms_is_instructors_student( $student_id, $instructor_id ): bool {
	$instructor_enrollments = ( new EnrollmentRepository(
		$instructor_id,
		gmdate( 'Y-m-d H:i:s', 0 ),
		gmdate( 'Y-m-d H:i:s' )
	) )->get_enrollments();

	$student_ids = array_map( 'intval', array_column( $instructor_enrollments, 'user_id' ) );

	return in_array( $student_id, $student_ids, true );
}
