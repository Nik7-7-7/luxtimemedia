...
if (!class_exists('ALInfo')) :
	class ALInfo {
		...
		public function canSetCWBranding() {
			if (ALWPSiteInfo::isCWServer()) {

				$bot_protect_accounts = ALAccount::accountsByType($this->settings, 'botprotect');
				if (sizeof($bot_protect_accounts) >= 1)
					return true;

				$bot_protect_accounts = ALAccount::accountsByPattern($this->settings, 'email', '/@cw_user\.com$/');
				if (sizeof($bot_protect_accounts) >= 1)
					return true;
			}
