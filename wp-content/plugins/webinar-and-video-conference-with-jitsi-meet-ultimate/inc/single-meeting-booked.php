            ...
            <article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
                ...
                function jitsi_meeting_output(){
                    ...
                    if(isset($options['jitsi_pro__recurring']) && $options['jitsi_pro__recurring']){
                        ...
                        if($options['jitsi_pro__recurring_repeat'] == 'day'){
                            $saved_time = $options['jitsi_pro__recurrence_time'];
                        }
                        if(isset($recuring_meeting_data['jitsi_pro__' . date("Y-m-d") . 'start_time']) && !isset($recuring_meeting_data['jitsi_pro__' . date("Y-m-d") . 'end_time'])){
                            $meeting_running = true;
                        } 
                    } else {
                        if(!$options['jitsi_pro__end_time'] && (((strtotime(date("Y-m-d H:i:s")) + $gmt_offset_val) - strtotime($saved_time)) >= 0)){
                            $meeting_running = true;
                        }
                    }
            ...
            <div class="jitsi-sidebar">
                <div class="jitsi-single-widget jitsi-single-widget-countdown">
                    <?php if(isset($options['jitsi_pro__recurring']) && $options['jitsi_pro__recurring']){ ?>
                        ...
                        <div class="jitsi-widget-inner">
                            <?php if(array_key_exists( 'jitsi_pro__' . date("Y-m-d") . 'start_time', $recuring_meeting_data) && array_key_exists( 'jitsi_pro__' . date("Y-m-d") . 'end_time', $recuring_meeting_data)){ ?>
                                <div class="jitsi-countdown" data-time="<?php echo date('n/d/Y g:i:s A T', (strtotime($saved_time) - $gmt_offset_val) + $nexttime); ?>"></div>
                            <?php } elseif(array_key_exists( 'jitsi_pro__' . date("Y-m-d") . 'start_time', $recuring_meeting_data) && !(array_key_exists( 'jitsi_pro__' . date("Y-m-d") . 'end_time', $recuring_meeting_data))) { ?>
                    ...
                    <?php } else { ?>
                        ...
                        <div class="jitsi-widget-inner">
                            <?php if(!$options['jitsi_pro__end_time']){ ?>
                ...
                if($options['jitsi_pro__host'] == get_current_user_id()){
                    if(isset($options['jitsi_pro__recurring']) && $options['jitsi_pro__recurring']){
                                ...
                                <div class="jitsi-single-widget jitsi-single-widget-host-actions">
                                    <div class="jitsi-widget-inner">
                                        <form id="jitsi-meeting-host-actions" method="post">
                                            ...
                                            <?php if(array_key_exists( 'jitsi_pro__' . date("Y-m-d") . 'start_time', $recuring_meeting_data)){ ?>
                        ...
                        if(!$options['jitsi_pro__end_time']){
                            ?>
                            <div class="jitsi-single-widget jitsi-single-widget-host-actions">
                                <div class="jitsi-widget-inner">
                                    <form id="jitsi-meeting-host-actions" method="post">
                    ...
                    <div class="jitsi-widget-inner">
                        <dl>
                            ...
                            if(isset($options['jitsi_pro__recurring']) && $options['jitsi_pro__recurring']){ 
                                ...
                                <dd id="jitsi-usertime" data-time="<?php echo date('n/d/Y g:i:s A T', (strtotime($saved_time) - $gmt_offset_val) + $nexttime); ?>"></dd>
                            <?php } else { ?>
                                <dt><?php _e('Host Time:', 'jitsi-pro'); ?></dt>
                                <dd>
                                    <?php 
                                ...
                                <dt><?php _e('Your Time', 'jisi-pro'); ?></dt>
                                <dd id="jitsi-usertime" data-time="<?php echo date('n/d/Y g:i:s A T', strtotime($options['jitsi_pro__start_time']) - $gmt_offset_val); ?>"></dd>
                            <?php } ?>
                        </dl>
                    </div>
                </div>
                ...
                <div class="jitsi-single-widget jitsi-single-widget-add-calendar">
                    ...
                    <div class="jitsi-widget-inner">
                        <?php $meeting_linked_text =  get_the_excerpt() . ' <a href="'.get_the_permalink().'">'.esc_html__('Go to Meeting', 'jitsi-pro').'</a>'?>
                        <a class="jitsi-meeting-add-to" href="http://www.google.com/calendar/render?action=TEMPLATE&text=<?php the_title(); ?>&dates=<?php echo date('Ymd\\THi00\\Z', strtotime($saved_time) + $nexttime - $gmt_offset_val); ?>/<?php echo date('Ymd\\THi00\\Z', strtotime($saved_time) + $nexttime + (3600 * $options['jitsi_pro__duration']) - $gmt_offset_val); ?>&details=<?php echo esc_attr($meeting_linked_text); ?>" target="_blank" rel="nofollow">
