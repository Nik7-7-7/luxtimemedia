...
export let breakdanceFunctions: BuilderApi = {
	...
	classInputSelector:
		"#breakdance-class-input-search input",
	...
	addClassToElement: function (className, elementId) {
		...
		let classExists = Breakdance.stores.globalStore.cssSelectors.find((cssSelector) => {
		...
		if (!element.data.properties.settings) element.data.properties =  {...element.data.properties, settings: {}};
		if (!element.data.properties.settings.advanced) element.data.properties.settings = {...element.data.properties.settings, advanced: {}};
		if (!element.data.properties.settings.advanced.classes) element.data.properties.settings.advanced = {...element.data.properties.settings.advanced, classes: []};
	...
	isClassAlreadyActive: function (className) {
		return this._getActiveElement().data.properties?.settings?.advanced?.classes?.includes(className);
	},
	...
	_getElementById: function (elementId) {
		...
		console.log(breakdanceElements[elementId])
