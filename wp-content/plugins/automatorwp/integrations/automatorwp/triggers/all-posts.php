...
class AutomatorWP_AutomatorWP_All_Posts extends AutomatorWP_Integration_Trigger {
    ...
    public function hooks() {
        ...
        add_filter( 'automatorwp_get_all_posts_automation_sql', array( $this, 'get_sql' ), 10, 7 );
     ...
     */
    public function get_sql( $sql, $automation, $trigger, $count, $trigger_options, $posts_per_loop, $loop ) {
        ...
        if( $count ) {
            ...
            $offset = $loop * $posts_per_loop;

            // The normal SQL query
            return "SELECT p.ID FROM {$wpdb->posts} AS p {$joins} {$where} LIMIT $offset, {$posts_per_loop}";
        }
